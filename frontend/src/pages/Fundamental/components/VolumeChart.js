...
const VolumeChart = (prop) => {
  ...
  const fetchData = async () => {
    ...
    const volumeList = Object.values(responseJson.Volume);
    ...
    const dataList = newdateList.map((element, index) => (
      { date: element, price: volumeList[index] }
    ));
    setData(dataList);
  };
  ...
  return (
    <>
      <ChartName> Volume</ChartName>
      <div id="timeSeriesVolume"></div>
    </>
  );
};
